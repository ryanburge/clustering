...
evan$white <- Recode(evan$race, "1=1; else=0")

test <- select(evan, age, literal, white, attnd, male, tolerance, abortion, gaymarriage, gaysex, repubid, welfare, military, environment, drugs, educ)
...
ggplot(test, aes(dp$proj[,1], dp$proj[,2], color=factor(k$cluster)))+
  geom_point(pch=19,size=2)+ggtitle("Evangelical Cluster Visualization") + xlab("First Dimension") + ylab("Second Dimension")  +
  ...
  annotate(geom="text", x=3.5, y=-1, label="Cluster 1", color="black", size =6) +
  annotate(geom="text", x=3.55, y=-2.5, label="Cluster 1", color="black", size =6) +
  annotate(geom="text", x=-2.5, y=-1, label="Cluster 2", color="black", size =6) +
  annotate(geom="text", x=-2.5, y=-2.5, label="Cluster 3", color="black", size =6)

ggsave(file="evan_cluster.png", type = "cairo-png", width = 15, height = 10)
